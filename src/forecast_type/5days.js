...
const FiveDays = (props) => {
  ...
  useEffect(() => {
    var options = {
      ...
      params: {
        lat: countryInfoDict[props.country].lat,
        lon: countryInfoDict[props.country].lon,
      },
      headers: {
  ...
  return (
    <div className="App">
      <Header component="5days" country={country} />
      <h4 className="h4">
        City Name : {state.city_name} &nbsp;&nbsp; Latitude : {state.lat}{" "}
        &nbsp;&nbsp; Longitude : {state.lon}
      </h4>
      <table className="table">
        ...
        <tbody>
          {state.data.map((data) => (
            <tr>
              <td key={state.city_name}>{data.timestamp_local}</td>
              <td>{data.temp}</td>
